...
/// 友盟相关
public class SLUMServicer: NSObject, UMAnalyticsProtocol, UMShareProtocol, UMPushProtocol {
    
    public static let shared = SLUMServicer()
    
    /// 注册友盟
    /// - Parameter appKey: AppKey
    public func regist(_ appKey: String) -> SLUMServicer {
...
extension SLUMServicer {
    @objc public func handleOpenURL(url: URL, options: [UIApplication.OpenURLOptionsKey: Any]) -> Bool {
        UMSocialManager.default()?.handleOpen(url, options: options) ?? false
    }

    @objc public func handleUniversalLink(activity: NSUserActivity) -> Bool {
